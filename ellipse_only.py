...
class MMDetector:
    ...
    def __init__(self):
        ...
        # Ellipse detection parameters - Optimized
        self.min_radius = 16
        self.max_radius = 23
        ...
        # Axis ratio parameters
        self.min_axis_ratio = 0.5  # Minimum major-to-minor axis ratio
        self.max_axis_ratio = 1.5  # Maximum major-to-minor axis ratio
    ...
    def detect_ellipses(self, gray: np.ndarray) -> List[Tuple[Tuple[float, float], Tuple[float, float], float]]:
        """
        Detect ellipses (imperfect circles) in the grayscale image.
        ...
        try:
            ...
            for contour in contours:
                if len(contour) >= 5:
                    ...
                    if (self.min_radius <= min(major_axis, minor_axis) / 2 <= self.max_radius and
    ...
    # Complete the image processing by detecting the circles and assigning the correct colours
    def process_image(self, gray: np.ndarray, hsv: np.ndarray) -> Tuple[np.ndarray, List[str]]: 
        ...
        try:
            ...
            colors = self.identify_colors(hsv, ellipses) if len(ellipses) > 0 else []
...
class MMCounterGUI: # All this is the GUI that allows effective interaction with the model and images with ease
    ...
    def setup_control_panel(self):
        ...
        try:
            ...
            # Axis ratio settings
            ttk.Label(settings_frame, text="Min Axis Ratio:").grid(row=1, column=0, padx=2)
            self.min_axis_ratio_var = tk.StringVar(value=str(self.detector.min_axis_ratio))
            ttk.Entry(settings_frame, textvariable=self.min_axis_ratio_var, width=5).grid(row=1, column=1, padx=2)
            
            ttk.Label(settings_frame, text="Max Axis Ratio:").grid(row=1, column=2, padx=2)
            self.max_axis_ratio_var = tk.StringVar(value=str(self.detector.max_axis_ratio))
            ttk.Entry(settings_frame, textvariable=self.max_axis_ratio_var, width=5).grid(row=1, column=3, padx=2)
    ...
    def apply_settings(self):
        ...
        try:
            ...
            messagebox.showerror("Error", "Invalid axis ratio values")
